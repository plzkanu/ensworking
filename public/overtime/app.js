...
function extractRecs(texts, cellAtIdx, label) {
  ...
  for (let i = 0; i < texts.length - 5; i++) {
    ...
    // 판별 우선순위: ① 휴일/주말이면 휴일조기출근 ② 평일이면 평일야간출근
    // (기존 09시 기준은 휴일/주말 여부로 대체 — 시간만으론 오판 多)
    ...
    if (bothV) {
      ...
      const dow = date ? new Date(date).getDay() : -1; // 0=일,6=토
    ...
    // 휴게시간 포함으로 종료시간이 (시작+시간수)보다 늦은 경우 보정
    ...
    const adjustedEnd = (() => {
      ...
      const [eh, em] = end_t.split(':').map(Number);
      const startMins = sh * 60 + sm;
      let endMins = eh * 60 + em;
      if (endMins < startMins) endMins += 24 * 60; // 자정 넘는 경우
      // 야간(22:00 이후)에 걸치는 근무는 종료시각을 앞당기면 야간시간(22:00~06:00)이
      // 줄어들어 배수가 틀어진다. 이 경우 종료를 보정하지 않고 원래 종료시각을 유지하고,
      // 실제 차감된 휴게는 splitRecord가 중간 공백으로 처리하도록 넘긴다.
      if (endMins > 22 * 60) return end_t;
      const calcEnd = startMins + Math.round(hrsVal * 60);
      if (endMins > calcEnd) {
        const h = Math.floor(calcEnd / 60) % 24;
        const m = calcEnd % 60;
    ...
    recs.push({ seq, name, empno, date, start, end: snapEnd(adjustedEnd),
...
function splitRecord(r) {
  ...
  if (remaining > 0) {
    ...
    if (nights.length) {
      ...
      const nightStarts = new Set(nights.map(n => n[0]));
...
// ── 유연근무 시간대 변환 (ERP 제출 전용) ───────────────────────────
// 유연근무제 사업소는 개인별 정규근무시간이 달라, 실제로는 시간외인데도
// 시계상 09:00~18:00 안에 위치하는 경우가 생긴다. 회사 ERP는 이 구간을
// 절대 시간외로 인정하지 않으므로(사업소 확인됨), 실제 시간외 총량은
// 그대로 두고 표기 시간대만 22시(야간근로 개시 전) 전까지 18:00~22:00
// → 06:00~09:00 순서로 옮겨서 ERP에 제출한다. 하루 변환 한도는 4h+3h=7h.
// 한도를 넘는 잔여분은 09-18 표기 그대로 두고 별도 경고로만 알린다.
// 원본 RECORDS는 절대 변경하지 않고, ERP 엑셀 생성 시에만 복제본에 적용한다.
function isConvertibleWeekday(dateStr) {
  if (!dateStr) return false;
  const dt = new Date(dateStr + 'T00:00:00');
  const dow = dt.getDay();
  const ch = getCustomHolidays();
  return dow !== 0 && dow !== 6 && !HOLIDAYS[dateStr] && !ch[dateStr];
}

function buildERPRecords(records) {
  const W1822 = [18 * 60, 22 * 60]; // 18:00~22:00
  const W0609 = [6 * 60, 9 * 60];   // 06:00~09:00
  const occ = {}; // `${empno}|${date}` -> { w1822:[[s,e],...], w0609:[[s,e],...] } (점유 구간, 정렬됨)
  const keyOf = (empno, date) => (empno || '') + '|' + date;
  const getOcc = key => occ[key] || (occ[key] = { w1822: [], w0609: [] });

  const addInterval = (list, s, e) => { list.push([s, e]); list.sort((a, b) => a[0] - b[0]); };

  // 주어진 창[winS,winE) 안에서 이미 점유된 구간을 뺀 빈 구간 목록
  const freeGaps = (list, winS, winE) => {
    const gaps = [];
    let cur = winS;
    for (const [s, e] of list) {
      if (s > cur) gaps.push([cur, Math.min(s, winE)]);
      cur = Math.max(cur, e);
      if (cur >= winE) break;
    }
  ...
  // 빈 구간을 앞에서부터 채워 need(분)만큼 배치, 다 못 채우면 남은 분량 반환
  const allocate = (list, [winS, winE], need) => {
    const placed = [];
    let remaining = need;
    for (const [gS, gE] of freeGaps(list, winS, winE)) {
      if (remaining <= 0) break;
      const take = Math.min(gE - gS, remaining);
      if (take > 0) {
        placed.push({ s: gS, e: gS + take });
        addInterval(list, gS, gS + take);
        remaining -= take;
      }
    }
  ...
  // dateStr과 같은 달(月) 안에서, 변환 가능한(평일·비휴일) 다른 날짜를 가까운 순으로 나열
  function nearbyConvertibleDates(dateStr) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (isConvertibleWeekday(ds)) cands.push(ds);
    }
    cands.sort((a, b) => {
      const da = Math.abs(Number(a.split('-')[2]) - dNum);
      const db = Math.abs(Number(b.split('-')[2]) - dNum);
      return da !== db ? da - db : a.localeCompare(b);
    });
    return cands;
  }

  // 1) 원래부터 18-22 / 06-09에 걸쳐 있던(변환 불필요) 실제 구간을 먼저 점유 등록
  records.forEach(r => {
    if (!isConvertibleWeekday(r.date)) return;
    const s = toMin(r.start);
    let e = toMin(r.end);
    if (e <= s) e += 1440;
    const o = getOcc(keyOf(r.empno, r.date));
    const ov1822s = Math.max(s, W1822[0]), ov1822e = Math.min(e, W1822[1]);
    if (ov1822e > ov1822s) addInterval(o.w1822, ov1822s, ov1822e);
    const ov0609s = Math.max(s, W0609[0]), ov0609e = Math.min(e, W0609[1]);
    if (ov0609e > ov0609s) addInterval(o.w0609, ov0609s, ov0609e);
  });
  ...
  records.forEach((r, srcIndex) => {
    const s = toMin(r.start);
    let e = toMin(r.end);
    if (e <= s) e += 1440;

    if (!isConvertibleWeekday(r.date)) { out.push(r); return; }

    const hidS = Math.max(s, 9 * 60), hidE = Math.min(e, 18 * 60);
    const hidden = Math.max(0, hidE - hidS);
    if (hidden === 0) { out.push(r); return; }

    const beforeSeg = hidS > s ? { s, e: hidS } : null; // 09:00 이전(그대로 인정)
    const afterSeg  = e > hidE ? { s: hidE, e } : null; // 18:00 이후(그대로 인정)

    let remain = hidden;
    const placedSegs = []; // { s, e, date } — date는 배치된(변환된) 실제 날짜

    // 1) 당일 18:00~22:00 → 06:00~09:00 순으로 배치
    const oSame = getOcc(keyOf(r.empno, r.date));
    let alloc = allocate(oSame.w1822, W1822, remain);
    alloc.placed.forEach(p => placedSegs.push({ ...p, date: r.date }));
    remain = alloc.remaining;
    if (remain > 0) {
      alloc = allocate(oSame.w0609, W0609, remain);
      alloc.placed.forEach(p => placedSegs.push({ ...p, date: r.date }));
      remain = alloc.remaining;
    }

    // 2) 당일 한도(7h) 초과분은 같은 달 내 가까운 다른 평일에서 빈 자리를 찾아 이어서 배치
    if (remain > 0) {
      for (const d2 of nearbyConvertibleDates(r.date)) {
        if (remain <= 0) break;
        const o2 = getOcc(keyOf(r.empno, d2));
        let a2 = allocate(o2.w1822, W1822, remain);
        a2.placed.forEach(p => placedSegs.push({ ...p, date: d2 }));
        remain = a2.remaining;
        if (remain > 0) {
          a2 = allocate(o2.w0609, W0609, remain);
          a2.placed.forEach(p => placedSegs.push({ ...p, date: d2 }));
          remain = a2.remaining;
        }
      }
    ...
    // remain > 0(같은 달 전체에서도 빈 자리를 못 찾은 극단적인 경우)는 09-18 그대로
    // 엑셀에 넣어봤자 ERP가 어차피 거부하므로 아예 제출하지 않고, overflow 경고로만 알린다.
    const segList = [];
    if (beforeSeg) segList.push({ ...beforeSeg, date: r.date, kind: 'orig' });
    placedSegs.forEach(p => segList.push({ ...p, kind: 'converted' }));
    if (afterSeg) segList.push({ ...afterSeg, date: r.date, kind: 'orig' });
    segList.sort((a, b) => a.date === b.date ? a.s - b.s : a.date.localeCompare(b.date));

    segList.forEach(seg => {
      const hrs = Math.round((seg.e - seg.s) / 60 * 10) / 10;
      out.push({ ...r, date: seg.date, start: toHHMM(seg.s), end: toHHMM(seg.e), hours: hrs, _converted: seg.kind === 'converted' });
    });

    if (placedSegs.length) {
      const sortedPlaced = [...placedSegs].sort((a, b) => a.date === b.date ? a.s - b.s : a.date.localeCompare(b.date));
      converted.push({
        srcIndex, name: r.name, empno: r.empno, date: r.date, from: `${r.start}~${r.end}`,
        parts: sortedPlaced.map(p => ({ date: p.date, start: toHHMM(p.s), end: toHHMM(p.e) })),
      });
    }
    if (remain > 0) {
      overflow.push({ srcIndex, name: r.name, date: r.date, remainMin: remain });
    }
  });

  return { records: out, converted, overflow };
}
...
function downloadErrList(ev) {
  ...
  XLSX.writeFile(wb, `시간외근무_유연근무_오류내역_${stamp}.xlsx`, { bookType:'xlsx', type:'binary', cellStyles:true });
}
...
function getParaRecordsAll(buf) {
  ...
  while (i + 4 <= buf.length) {
    ...
    if (tagId === 67) {
      ...
      recs.push({ headerOff, extSizeOff, dataStart, end, text: s.trim() });
    }
    i = end;
  }
  return recs;
...
function applyRowBlank(buf, rb, contentCellIdx) {
  ...
  for (let m = 0; m + 5 < allRecs.length; m++) {
    ...
    // (이 파일의 getParaRecordsAll은 {dataStart,end,text} 형태라 patchRecord가 쓰는 start/end로 맞춰준다)
    for (const idx of [m, m+1, m+2, m+3, m+4, m+5]) {
      const t = allRecs[idx].text;
      if (t) patchRecord(buf, { start: allRecs[idx].dataStart, end: allRecs[idx].end }, strU16le(t), strU16le(' '.repeat(t.length)));
    }
    ...
    if (m > 0 && /^\d{1,3}$/.test(allRecs[m - 1].text)) {
      ...
      patchRecord(buf, { start: allRecs[m-1].dataStart, end: allRecs[m-1].end }, strU16le(st), strU16le(' '.repeat(st.length)));
    }
    break;
  }
}
...
async function parseAll() {
  ...
  if (RECORDS.length) {
    ...
    for (const r of RECORDS) {
      ...
      if (isHoliday || dow === 5) continue; // 휴일, 금요일은 대상 아님
      const s = toMin(r.start);
      let e = toMin(r.end);
      if (e <= s) e += 1440;
      const hidS = Math.max(s, 9 * 60), hidE = Math.min(e, 18 * 60);
...
function downloadERPExcel() {
  const ch = getCustomHolidays();
  const warns = [];
  // 휴게시간 분리(splitRecord)로 같은 근무 건이 여러 행으로 쪼개져도, 같은 사람+날짜+유형은
  // 한 번만 경고에 뜨도록 중복 제거
  const seenWarnKeys = new Set();
  const pushOnce = (empno, date, type, warnObj) => {
    const key = `${empno}|${date}|${type}`;
    if (seenWarnKeys.has(key)) return;
  ...
  RECORDS.forEach((r, idx) => {
    ...
    const holMissing = isHoliday && isBefore9 && !hol;
    const holWrong   = hol && (!isHoliday || !isBefore9);
    if (holMissing)  pushOnce(r.empno, r.date, 'holMissing', { idx, r, type: 'holMissing',  label: '휴일조기출근 누락 가능',     desc: `${r.name} (${r.date}) — 휴일 09:00 이전 출근인데 V 체크 없음`, color: 'var(--red)' });
    if (holWrong)    pushOnce(r.empno, r.date, 'holWrong',   { idx, r, type: 'holWrong',    label: '휴일조기출근 오체크 가능',   desc: `${r.name} (${r.date}) — 평일이거나 09:00 이후인데 V 체크됨`, color: 'var(--red)' });
    if (night)       pushOnce(r.empno, r.date, 'night',      { idx, r, type: 'night',       label: '평일야간출근 V 체크 확인',   desc: `${r.name} (${r.date}) — 평일야간출근 체크 항목`, color: '#7C3AED' });
  });

  const { converted, overflow } = buildERPRecords(RECORDS);
  converted.forEach((c, i) => {
    const toText = c.parts.map(p => `${p.date} ${p.start}~${p.end}`).join(', ');
    warns.push({ idx: `conv${i}`, r: null, type: 'converted', label: '유연근무 시간대 변환',
      desc: `${c.name} — 원본: ${c.date} ${c.from} &rarr; 변환: ${toText}`,
      color: 'var(--blue)' });
  });
  overflow.forEach((o, i) => {
    warns.push({ idx: `ovf${i}`, r: null, type: 'convertOverflow', label: '변환 한도 초과 (수기 확인 필요)',
      desc: `${o.name} (${o.date}) — 하루 변환 가능 한도(7시간)를 초과해 ${o.remainMin}분을 자동 변환하지 못했습니다. 해당 건은 09-18 시간대로 그대로 제출되어 ERP에서 인정되지 않으니 수기로 처리해주세요.`,
      color: 'var(--red)' });
  });
...
function doDownloadERPExcel() {
  ...
  // 유연근무로 09-18 내부에 숨은 시간외는 여기서 ERP 인정 시간대로 변환된 값을 사용
  const { records: erpRecords } = buildERPRecords(RECORDS);
  ...
  const sDataConv = { // 유연근무 시간대 변환이 적용된 시작/종료 셀 (노란색 강조)
    font:      {name:'맑은 고딕', sz:10},
    fill:      {fgColor:{rgb:'FFF3B0'}, patternType:'solid'},
    border:    borderGray,
    alignment: {vertical:'center'},
  };
  const sHoursConv = { // 유연근무 시간대 변환이 적용된 시간수 셀 (노란색 강조)
    font:      {name:'맑은 고딕', sz:10},
    numFmt:    '#,##0.0',
    fill:      {fgColor:{rgb:'FFF3B0'}, patternType:'solid'},
    border:    borderGray,
  ...
  personBlocks.forEach(block => {
    ...
    block.slots.forEach((entries, si) => {
      ...
      dates.forEach((date, di) => {
        ...
        if (rec) {
          // 유연근무 시간대 변환이 적용된 셀은 노란색으로 강조 표시
          const dataStyle  = rec._converted ? sDataConv  : sData;
          const hoursStyle = rec._converted ? sHoursConv : sHours;
          setCell(r, colIdx(di,0), rec.start || '', dataStyle);
          setCell(r, colIdx(di,1), rec.end   || '', dataStyle);
          setCell(r, colIdx(di,2), rec.hours || 0,  hoursStyle);
  ...
  const now = new Date();
  const stamp = `${now.getFullYear()}${String(now.getMonth()+1).padStart(2,'0')}${String(now.getDate()).padStart(2,'0')}`;
  XLSX.writeFile(wb, `유연_시간외근무신청(ERP)_${stamp}.xlsx`, {bookType:'xlsx', type:'binary', cellStyles:true});
}
...
function render(){
  ...
  // 유연근무 시간대 변환 미리보기 (ERP 다운로드 시 실제 적용될 결과와 동일한 계산)
  const { converted: convPreview, overflow: ovfPreview } = buildERPRecords(RECORDS);
  const convByIdx = new Map(convPreview.map(c => [c.srcIndex, c]));
  const ovfByIdx = new Map(ovfPreview.map(o => [o.srcIndex, o]));
  ...
  document.getElementById('tBody').innerHTML=fl.map((r,i)=>{
    ...
    // 유연근무 시간대 변환 미리보기 배지 (ERP 다운로드 시 이 시간대로 바뀌어 제출됨)
    const convInfo = convByIdx.get(r._i);
    const ovfInfo = ovfByIdx.get(r._i);
    const convBadge = convInfo
      ? `<div class="tip-wrap" style="display:inline-block;margin-left:4px">
           <span class="tag" style="background:#EFF6FF;color:#2563EB;border:1px solid #93C5FD;font-size:10px;cursor:default">→ 변환</span>
           <div class="tip">ERP 제출 시 아래로 변환됩니다:<br>${convInfo.parts.map(p=>`${p.date} ${p.start}~${p.end}`).join('<br>')}${ovfInfo?`<br><br><span style="color:#F04452">⚠️ ${ovfInfo.remainMin}분은 변환 한도 초과로 자동 변환 실패 — 수기 처리 필요</span>`:''}</div>
         </div>`
      : (ovfInfo
        ? `<div class="tip-wrap" style="display:inline-block;margin-left:4px">
             <span class="tag tag-warn" style="font-size:10px;cursor:default">⚠️ 변환초과</span>
             <div class="tip">이번 달 전체에서 빈 자리를 찾지 못해 ${ovfInfo.remainMin}분이 자동 변환되지 않았습니다.<br>ERP 제출에서 제외되니 수기로 처리해 주세요.</div>
           </div>`
        : '');
    ...
    return`<tr style="${r._split && r._splitIdx > 0 ? 'background:#FAFAFF' : ''}">
      ...
      <td><span class="ef">${r.end}</span>${convBadge}</td>
